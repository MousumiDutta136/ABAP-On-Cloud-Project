CLASS zrap_md_data_uploader_cl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zrap_md_data_uploader_cl IMPLEMENTATION.
METHOD fill_master_data.
    data : lt_bp type table of zrap_md_bpdb,
           lt_prod type table of zrap_md_product.
   ...
   append value #(
    ...
    append value #(
     ...
     insert zrap_md_bpdb from table @lt_bp.
     insert zrap_md_product from table @lt_prod.
  ENDMETHOD.
  ...
  METHOD fill_transaction_data.
    data : o_rand type REF TO cl_abap_random_int,
           ...
           lv_ord_id type zrap_md_dte_id,
           lt_so type table of zrap_md_so_hdr,
           lt_so_i type table of zrap_md_so_itm.
    ...
    select * from zrap_md_bpdb into table @data(lt_bpa).
    select * from zrap_md_product into table @data(lt_prod).
    ...
    insert zrap_md_so_hdr from table @lt_so.
    insert zrap_md_so_itm from table @lt_so_i.

  ENDMETHOD.

  METHOD flush.
    delete from : zrap_md_bpdb, zrap_md_product, zrap_md_so_hdr, zrap_md_so_itm.
  ENDMETHOD.
